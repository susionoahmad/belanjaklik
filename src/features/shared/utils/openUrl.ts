...
export function openUrl(url: string, options: OpenUrlOptions = {}): boolean {
  ...
  const fallback = options.fallbackToCurrentTab !== false;
  ...
  try {
    ...
    if (!newWin || newWin.closed || typeof newWin.closed === 'undefined') {
      console.warn('[openUrl] Window popup was blocked by browser. Falling back to current location.');
      if (fallback) {
 ...
 * If popup opening is blocked, falls back to navigating the current tab.
 */
export async function openUrlAsync(
  ...
  try {
    ...
    if (popupWin && popupWin.document) {
      popupWin.document.title = 'Mengarahkan ke WhatsApp...';
      ...
      popupWin.document.body.innerHTML = `
        <div style="text-align: center; padding: 24px; max-width: 360px; background: white; border-radius: 16px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.1); border: 1px solid #e2e8f0;">
          <div style="width: 36px; height: 36px; border: 3px solid #e2e8f0; border-top-color: #10b981; border-radius: 50%; animation: spin 0.8s linear infinite; margin: 0 auto 16px;"></div>
          <div style="font-size: 15px; font-weight: 700; color: #0f172a;">Menyiapkan WhatsApp...</div>
