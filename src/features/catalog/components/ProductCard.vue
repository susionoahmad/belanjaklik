<template>
  <div 
    class="bg-white dark:bg-gray-800 rounded-3xl p-3 sm:p-4 border border-gray-100 dark:border-gray-700 shadow-soft hover:shadow-hover transition-all duration-300 flex flex-col justify-between group relative overflow-hidden cursor-pointer"
    @click="$emit('select', product)"
  >
    <!-- Top Badges -->
    <div class="absolute top-3 left-3 z-10 flex flex-wrap gap-1">
      <span v-if="product.is_promo && product.promo_price" class="bg-gradient-to-r from-red-600 to-rose-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-sm uppercase tracking-wider">
        PROMO
      </span>
    ...
    <!-- Product Image -->
    <div class="relative w-full aspect-square rounded-2xl overflow-hidden bg-gray-50 dark:bg-gray-700/60 mb-3 group-hover:scale-102 transition-transform duration-300 flex items-center justify-center p-1.5">
      <img 
        :src="displayImageUrl" 
        :alt="product.name" 
    ...
    <div class="mt-3 pt-2 border-t border-gray-100 dark:border-gray-700/60" @click.stop>
      ...
      <div v-if="product.purchase_method === 'owner_checkout' && cartQuantity > 0" class="flex items-center justify-between bg-red-50 dark:bg-red-950/40 rounded-2xl p-1 border border-red-100 dark:border-red-900">
        <button 
