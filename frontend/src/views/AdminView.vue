    ...
    <div v-else class="space-y-5">
      ...
      <div v-else-if="activeTab === 'settings'" class="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 space-y-4 max-w-xl shadow-soft">
        <div class="flex items-center justify-between border-b border-gray-100 dark:border-gray-700 pb-3">
          ...
          <span class="text-[11px] text-emerald-600 font-semibold bg-emerald-50 dark:bg-emerald-950 px-2.5 py-1 rounded-full">Tersimpan ke Database</span>
        </div>
        ...
        <form @submit.prevent="handleSaveProfile" class="space-y-3.5">
          ...
          <div class="pt-3">
            ...
            >
              <Save class="w-4 h-4" />
              <span>Simpan Profil Toko Ke Database</span>
            </button>
          </div>
        </form>
      </div>
...
const filteredAdminProducts = computed(() => {
  // Kelola Produk hanya menampilkan produk milik sendiri; affiliate dikelola di tab khusus.
  let list = catalogStore.products.filter(p => p.purchase_method !== 'coming_soon');
