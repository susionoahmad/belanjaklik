...
const DEFAULT_CATEGORIES: Category[] = [
  ...
  { id: 'c7777777-7777-7777-7777-777777777777', name: 'Travel & Tiket Wisata', slug: 'travel-tiket-wisata', icon: 'plane', sort_order: 20 }
];
...
export const dataService = {
  ...
  async fetchCategories(): Promise<Category[]> {
    ...
    for (const cat of list) {
      ...
      for (const p of allProducts) {
        if (p.category_id === cat.id) { cnt++; continue; }
        if (p.category && (p.category.toLowerCase().includes(cName) || cName.includes(p.category.toLowerCase()))) { cnt++; continue; }
      }
      catCountMap.set(cat.id, cnt);
    }

    list.sort((a, b) => {
      const countA = catCountMap.get(a.id) || 0;
      const countB = catCountMap.get(b.id) || 0;
  ...
  async fetchProductSources(): Promise<ProductSource[]> {
    ...
    // Always fetch products catalog to match or generate product sources
    const allProducts = await this.fetchProducts();

    // Auto-generate ProductSources from catalog products if rawSources is empty
    if (rawSources.length === 0) {
      ...
      for (const p of allProducts) {
        if (p.product_type === 'own' || p.id.startsWith('own-') || p.external_product_code || (p.purchase_method as string) === 'alfamind_tokosaya' || (p.product_url && p.product_url.includes('tokovirtualku'))) {
          ...
          derived.push({
            id: `src_${p.id}`,
            product_id: p.id,
            source_type: 'tokosaya_alfamind',
            source_url: p.product_url || `https://tokovirtualku.id/nessamart/detail/${code}`,
            ...
            product_image_url: p.image_url || p.thumbnail_url || 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=150'
          } as unknown as ProductSource);
        }
      }
      if (derived.length > 0) {
        await offlineDb.setSources(derived);
        return derived;
      }
    }
  ...
  async saveProductSource(source: Partial<ProductSource>): Promise<ProductSource> {
    ...
    const cleanUrl = source.source_url ? source.source_url.trim() : '';

    const idx = sources.findIndex(s =>
      (source.id && s.id === source.id) ||
      (cleanUrl && s.source_url && s.source_url.trim() === cleanUrl) ||
      (source.external_product_code && s.external_product_code && s.external_product_code === source.external_product_code) ||
      (source.product_id && s.product_id && s.product_id === source.product_id)
    );
    ...
    if (idx !== -1) {
      ...
      const targetId = source.id || `src_${Date.now()}`;
      updated = { ...source, id: targetId } as ProductSource;
  ...
  async fetchProducts(): Promise<Product[]> {
    ...
    const inferCategoryIdByName = (name: string, categoryStr?: string): { id: string; name: string } => {
      ...
      // Travel & Tiket Wisata
      const travelKws = ['tiket', 'pura', 'heritage', 'wisata', 'tour', 'hotel', 'resort', 'flight', 'pesawat', 'amazone', 'taman', 'rekreasi', 'voucher', 'pesiar'];
      if (travelKws.some(kw => n.includes(kw))) {
        return { id: 'c7777777-7777-7777-7777-777777777777', name: 'Travel & Tiket Wisata' };
      }
    ...
    if (apiBaseUrl) {
      try {
        ...
        const res = await fetch(`${apiBaseUrl}/api/v1/products?type=own&limit=1000`, { signal: controller.signal });
        clearTimeout(timeoutId);
        if (res.ok) {
          const json = await res.json();
          if (json.data && Array.isArray(json.data) && json.data.length > 0) {
            baseList = json.data.map((item: any) => ({
              id: item.id,
              name: item.name,
              slug: item.slug,
              brand: item.brand,
              ...
              purchase_method: item.product_type === 'affiliate' ? 'coming_soon' : 'owner_checkout',
            }));
          }
        }
      } catch (err) {
        console.warn('Go Backend API fetch skipped or offline, attempting fallback', err);
      }
    }

    // 2. Fallback to Supabase Egress Cache if Go API returned empty
    ...
    if (baseList.length === 0 && isSupabaseConfigured && !isCacheValid) {
      try {
