...
export class ProductSyncService {
  ...
  static async syncProductUrl(sourceUrl: string, existingSourceId?: string): Promise<{ product: Product; source: ProductSource }> {
    ...
    // Look for existing ProductSource in database by ID, source_url, or external_product_code to UPDATE instead of insert duplicate
    const existingSources = (await dataService.fetchProductSources()) || [];
    const existingRecord = existingSources.find(s =>
      (existingSourceId && s.id === existingSourceId) ||
      (s.source_url && s.source_url.trim() === cleanUrl) ||
      (s.external_product_code && s.external_product_code === externalCode)
    );
    ...
    const targetSourceId = existingRecord?.id || existingSourceId || sourceUuid;
